// just replace the code after the List<Integer> breakingRecoreds(...)
import java.io.*;
import java.math.*;
...
class Result {
    ...
    public static List<Integer> breakingRecords(List<Integer> scores) {
        ...
        int maximumScoreCount =0;
        int minimumScoreCount = 0;
        
        ArrayList<Integer> answer= new ArrayList<>();
        
        for(int i=0; i<scores.size();i++){
            if(scores.get(i)>maximunScore){
                maximunScore = scores.get(i);
                maximumScoreCount++;
            }else if(scores.get(i)< minimumScore){
                minimumScore = scores.get(i);
                minimumScoreCount++;
            }
        }
        answer.add(maximumScoreCount);
        answer.add(minimumScoreCount);
        return answer;
    }
